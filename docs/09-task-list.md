...
- [ ] **T0.2** Create `melos.yaml` + the 8 package skeletons (`domain`, `intelligence`, `application`, `git`, `storage`, `registry`, `platform`, `cli`), each with the `lib/<name>.dart` barrel + `lib/src/` + `test/` convention — [06 §1](06-package-design.md)
- [ ] **T0.3** Shared `analysis_options.yaml`: `lints/recommended` + `prefer_final_locals`, `unawaited_futures`, `directives_ordering`, no-`print` — [08 §3](08-contributing-guide.md)
